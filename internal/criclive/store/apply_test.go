...
func TestLiveAdmissionRequiresFutureNotStartedFixture(t *testing.T) {
	...
	fixture.State = client.StateInProgress
	if futureNotStartedFixture(fixture, now.Add(time.Minute), now) {
		t.Fatal("active fixture was admitted")
	}
	fixture.State = client.StatePreview
	...
	fixture.State = "new-provider-phase"
	if futureNotStartedFixture(fixture, now.Add(time.Minute), now) {
...
func TestLiveAdmissionAllowsOnlyProviderLiveOrBreakStates(t *testing.T) {
	now := time.Date(2026, 7, 16, 12, 0, 0, 0, time.UTC)
	for _, state := range []string{"In Progress", "Innings Break", "Lunch", "Tea", "Drinks", "Stumps", "Rain"} {
		if !liveAdmissionAllowed(client.Fixture{State: state}, true, nil, now) {
			t.Fatalf("state %q was not admitted", state)
		}
	}
	for _, state := range []string{"", "Preview", "Scheduled", "Toss", "Delay", "Complete", "Abandon", "No Result", "new-provider-phase"} {
		if liveAdmissionAllowed(client.Fixture{State: state}, true, nil, now) {
			t.Fatalf("state %q was admitted", state)
		}
	}
	if liveAdmissionAllowed(client.Fixture{State: "Innings Break"}, false, nil, now) {
