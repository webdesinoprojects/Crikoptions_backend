...
var (
	ErrStaleSnapshot      = errors.New("CricLive snapshot is older than committed state")
	ErrCorrectionDisabled = errors.New("live CricLive corrections are disabled")
	ErrConcurrentApply    = errors.New("CricLive match changed during apply")
	ErrFixtureLeaseLost   = errors.New("CricLive fixture lease was lost before apply")
...
func (s *Store) ApplyProjection(ctx context.Context, projection reconcile.Projection, raw []byte, receivedAt time.Time, cfg ApplyOptions) (ApplyResult, error) {
	...
	if !ok {
		return ApplyResult{}, errors.New("invalid CricLive apply result")
	}
	return result, nil
}
...
func (s *Store) applyProjectionTransaction(ctx mongo.SessionContext, projection reconcile.Projection, receivedAt time.Time, cfg ApplyOptions) (ApplyResult, error) {
	...
	if !forceResolve && (firstMissing || correctionBlocked) {
		...
		if firstMissing {
			...
			if err := s.incrementMissingPolls(ctx, projection, existingEvents, candidateByID); err != nil {
				return ApplyResult{}, err
			}
		}
	...
	if forceResolve && (firstMissing || correctionBlocked) {
		log.Printf(
			"criclive fixture %d match %s: forcing delivery reconcile after %d polls (missing=%t corrections=%t terminal=%t)",
...
func withinDeliveryWindow(projection reconcile.Projection, event matches.BallEvent) bool {
	...
	if event.Innings != projection.DeliveryWindowInnings {
		return event.Innings > projection.DeliveryWindowInnings
	}
	return event.Sequence >= projection.DeliveryWindowSequence
}

func (s *Store) incrementMissingPolls(ctx context.Context, projection reconcile.Projection, existing map[string]matches.BallEvent, candidateByID map[string]reconcile.Delivery) error {
...
func (s *Store) applyEvents(ctx context.Context, matchID string, projection reconcile.Projection, existing map[string]matches.BallEvent, receivedAt time.Time) (int, int, []matches.BallEvent, error) {
	...
	for providerID, old := range existing {
		...
		// CricLive only republishes the most recent overs, so a stored ball from
		// earlier in the innings is simply out of view, not withdrawn. Only
		// deliveries inside the window the provider still covers may be
		// tombstoned.
		if !withinDeliveryWindow(projection, old) {
			continue
		}
...
func initialMatch(projection reconcile.Projection, now time.Time) matches.Match {
	return matches.Match{
		...
		TournamentID:    fmt.Sprintf("%s:%d", ProviderName, projection.LeagueID), Format: projection.Format,
		TeamAID:   fmt.Sprintf("%s:%d", ProviderName, projection.LocalTeamID),
		TeamBID:   fmt.Sprintf("%s:%d", ProviderName, projection.VisitorTeamID),
...
// progressiveMatchView rewinds the authoritative snapshot to the state just
// after `through` was bowled. CricLive exposes only the most recent overs, so
// the totals cannot be summed forward from the first delivery; they are instead
// unwound backwards from the projection's own figures, which are exact.
func progressiveMatchView(base matches.Match, projection reconcile.Projection, through reconcile.Delivery) matches.Match {
	next := base
	score, wickets, legal := projection.CurrentScore, projection.Wickets, projection.LegalBalls
	throughDeliveries := make([]reconcile.Delivery, 0, len(projection.Deliveries))
	for _, delivery := range projection.Deliveries {
		if delivery.Innings != through.Innings {
			continue
		}
		if !deliveryAfter(delivery, through) {
