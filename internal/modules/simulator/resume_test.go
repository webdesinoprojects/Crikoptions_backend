...
func TestDeriveResumePlan_innings2AfterInnings1Done(t *testing.T) {
	match := &matches.Match{Status: matches.StatusLive, Innings: 1, CurrentScore: 177}
	ds := &CSVDataset{
		HasInnings2: true,
		Events: map[int][]BallRow{
			1: make([]BallRow, 121),
...
func TestDeriveResumePlan_skipAllEventsApplied(t *testing.T) {
	match := &matches.Match{Status: matches.StatusLive, Innings: 2}
	ds := &CSVDataset{
		HasInnings2: true,
		Events: map[int][]BallRow{
			1: make([]BallRow, 5),
